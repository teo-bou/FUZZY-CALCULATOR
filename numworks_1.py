...
class class_grid:
    ...
    def travel_x(self, i):
        ...
        if i == 1:  # GO RIGHT
            ...
            for cell in self.__grid[y][:x][
                        ::-1]:  # On part notre cell et on va vers la gauche et focus le premier bouton.
                if cell is not None:
                    self.focus_cell(cell)
                    return
            return
...
class class_liste_principale(class_grid):
    ...
    def travel_y(self, i):
        y = self.focused[1]
        if y == 0 and i == -1 and self.rows[0][0].grid_coordinates[
            1] != 0:  # On est en haut et on veut monter et il y a des boutons au dessus
            print("GO DOWN")
            self.go_down()
...
class class_interface():
    def __init__(self, grid: class_liste_principale, menu=class_grid):
        ...
        self.text_mode_actions = {
            ...
            "lettres": [KEY_ONE, KEY_TWO, KEY_THREE, KEY_FOUR, KEY_FIVE, KEY_SIX, KEY_SEVEN, KEY_EIGHT, KEY_NINE,
                        KEY_ZERO, KEY_ANS, KEY_DOT, KEY_SHIFT]
        }
